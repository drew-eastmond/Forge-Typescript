...
var LibraryPackageValidate = class extends AsyncArgumentPackageValidate {
  async $flush(query, validations) {
    ...
    if (ext === void 0) validations.add(
      new ArgumentValidationWarning(`[${this.constructor.name}] arguments for <warning>{ ext }</warning> is optional and will default to = <blue>".js"`, { cause }),
      { ext: true, ...warningAttributes }
    );
    if (validations.errors.length === 0) validations.add(
      new ArgumentValidationSuccess(`[${this.constructor.name}] package validated for Library`),
