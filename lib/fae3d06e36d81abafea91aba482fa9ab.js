...
var BundlePackageValidate_exports = {};
__export(BundlePackageValidate_exports, {
  LibraryPackageValidate: () => LibraryPackageValidate
});
module.exports = __toCommonJS(BundlePackageValidate_exports);
...
var LibraryPackageValidate = class extends import_forge.AsyncArgumentPackageValidate {
  async $flush(query, validations) {
    ...
    if (ext === void 0) validations.add(
      new import_forge.ArgumentValidationWarning(`[${this.constructor.name}] arguments for <error>{ ext }</warning> is optional and will default to = <blue>".js"`, { cause }),
    ...
    if (validations.errors.length === 0) validations.add(
      new import_forge.ArgumentValidationSuccess(`Package validated for Bundle`),
