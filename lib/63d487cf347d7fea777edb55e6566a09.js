...
var ForgeBuilder_exports = {};
__export(ForgeBuilder_exports, {
  ForgeBuilder: () => ForgeBuilder
});
module.exports = __toCommonJS(ForgeBuilder_exports);
var import_forge = require("@onyx-ignition/forge");
var import_TypescriptFile = require("./e6dd33bd637454f0c111b7dac72dc980.js");
var ReorderManager = class {
  _root;
  // private _$fetch
  topology = new import_forge.Topology();
  ...
  async $load(file, spaces) {
    const hierarchy = [];
    const input = await import_forge.ForgeFile.$ReadDecoded(file, "utf8");
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (result) {
        ...
        const file2 = import_forge.ForgePath.Sanitize(this._root, result.groups.file);
  ...
  add(file, attributes, parent) {
    this.topology.add(import_forge.ForgePath.Sanitize(this._root, file), attributes, parent);
    return this;
  }
};
var ForgeBuilder = class extends import_forge.Subscription {
  ...
  async _$fetch(file) {
    ...
    if (this.cache.has(file)) {
      ...
      const contents = await import_forge.ForgeFile.$ReadDecoded(file, "utf8");
  ...
  async $bundle(entry) {
    ...
    const prefix = "Forge_";
    const suffix = "";
    const entryFile = new import_TypescriptFile.TypescriptFile({ entry, root: this.root }, { $fetch: this._$fetchTypescript, externals: this._builderOptions.externals });
    ...
    for (const [file, components] of bundledImports) {
      if (import_forge.ForgePath.IsAbsolute(file) === false) {
        console.blue(file);
