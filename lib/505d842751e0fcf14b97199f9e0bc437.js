...
var BuildClient = class extends import_forge.ForgeClient {
  ...
  async $library(data, session) {
    ...
    if (build.bundled == import_Core.Bundle.mangle_static || build.bundled == import_Core.Bundle.mangle_random || build.bundled == import_Core.Bundle.preserve) {
      return SerializeResult(await library.$export({ files, root }, new import_Core.BuilderConfig(build), { lib: "./lib/", ext: ".js" }));
    } else if (build.bundled == import_Core.Bundle.merge) {
  ...
  async $types(data, session) {
    ...
    return SerializeResult(await (0, import_TypesBuilder.$BuildTypes)(entry, name, build));
  }
};
