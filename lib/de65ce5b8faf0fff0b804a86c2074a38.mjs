...
import { $Strip, $Transform, BuilderConfig } from "./fda017fd934f5aa37610fc2a70164a5e.mjs";
...
"${file}"`);
  ...
  async $strip(callback) {
    const output = [];
    return $Strip({ code: this._code, root: this._root }, new BuilderConfig({ format: "forge-ts", externals: this._externals }));
  }
